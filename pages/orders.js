        ...
        <div className="overflow-auto h-[58vh] shadow-box w-[95vw] md:mr-10 md:h-[80vh] md:w-[82vw] md:ml-36 xl:w-[88vw]">
          ...
          >
            ...
            <tbody>
              {orderData.map((order) => {
                return (
                  <tr key={order.order_uid} id={`${order.order_uid}-row`}>
                    <td>
                      <button
                        className="flex pl-3 w-10"
                        onClick={() => {
                          openDeleteModal(order.order_uid, order.customer_uid);
                        }}
                      >
                        <svg
                          width="24"
                    ...
                    <td>
                      <OrderTableData
                        ...
                        prepend="$"
                      >
                        {order.order_cost + order.misc_fees}
                      </OrderTableData>
                    </td>
                    <td>
                      <OrderTableData
                      ...
                      >
                        {new Date(order.creation_timestamp).toDateString()}
                      </OrderTableData>
                    </td>

                    {productNames.map((product) => {
                      const productQuantity =
                        order.order_items?.[product.product_id];
                      return (
